...
void ScrollingFrame::rightStickScrollingBehaviour()
{
    ...
    const float viewHeight = getScrollingAreaHeight();
    if (contentHeight <= viewHeight + 0.5f)
        return;

    const auto& state = Application::getControllerState();
    const float axis = state.axes[RIGHT_Y];
    constexpr float DEAD_ZONE = 0.22f;

    if (std::fabs(axis) <= DEAD_ZONE)
        return;

    // Preserve fine movement near the dead zone while still allowing fast
    // traversal through long pages at full deflection.
    const float normalized =
        (std::fabs(axis) - DEAD_ZONE) /
        (1.0f - DEAD_ZONE);
    const float signedAmount =
        axis < 0.0f ? -normalized : normalized;
    const float frameRate =
        std::max(30.0f, static_cast<float>(Application::getFPS()));
    const float pixelsPerFrame =
        1500.0f / frameRate;

    setContentOffsetY(
        getContentOffsetY() +
            signedAmount * pixelsPerFrame,
        false);
}
