...
{
    if (!(this->alpha > 0.0f && this->collapseState != 0.0f &&
          this->highlightAlpha > 0.0f && !this->hideHighlightBorder &&
          !this->hideHighlight))
        return;
    ...
    // Highlight borders are rendered globally after every activity, so the
    // normal scissor used while drawing a scrolling frame no longer exists at
    // this point. Rebuild the clipping chain from the focused view's ancestors.
    //
    // Clip only on the axis each scrolling view actually scrolls. A full
    // rectangle clip on HScrollingFrame cut off the cyan border's top and
    // bottom edges whenever a card filled the row height. Vertical clipping on
    // ScrollingFrame still keeps home-page cards behind the fixed upper panel,
    // while horizontal clipping keeps carousel cursors inside their row.
    for (View* ancestor = this->getParent(); ancestor;
         ancestor = ancestor->getParent())
    {
        Rect clip = ancestor->getFrame();

        if (dynamic_cast<ScrollingFrame*>(ancestor))
        {
            nvgIntersectScissor(
                ctx->vg,
                0.0f,
                clip.getMinY(),
                static_cast<float>(Application::contentWidth),
                clip.getHeight());
        }
        else if (dynamic_cast<HScrollingFrame*>(ancestor))
        {
            nvgIntersectScissor(
                ctx->vg,
                clip.getMinX(),
                0.0f,
                clip.getWidth(),
                static_cast<float>(Application::contentHeight));
        }
        else if (ancestor->getClipsToBounds())
        {
            nvgIntersectScissor(
                ctx->vg,
                clip.getMinX(),
