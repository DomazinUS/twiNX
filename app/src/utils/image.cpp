...
void Image::with(brls::Image* view, const std::string& url) {
    ...
    item->image = view;
    item->url = url;
    item->isCancel = std::make_shared<std::atomic_bool>(false);
    view->ptrLock();
...
void Image::doRequest(HTTP& s) {
    const auto imagePtr = this->image;
    const auto urlCopy = this->url;
    const auto requestToken = this->isCancel;

    if (!imagePtr || !requestToken || requestToken->load()) {
        Image::clear(imagePtr, requestToken);
        return;
    }

    try {
        std::ostringstream body;
        HTTP::set_option(s, requestToken, HTTP::Timeout{});
        s._get(urlCopy, &body);
...
#ifdef USE_WEBP
        char* ct = nullptr;
        if (urlCopy.find("Webp") != std::string::npos ||
            (s.getinfo(&ct) &&
             ct != nullptr &&
             strcmp(ct, "image/webp") == 0)) {
        ...
        brls::sync([imagePtr, urlCopy, requestToken, imageData, imageW, imageH, isWebp] {
            if (!requestToken->load()) {
                int tex = brls::TextureCache::instance().getCache(urlCopy);
                if (tex == 0 && imageData != nullptr) {
                    NVGcontext* vg = brls::Application::getNVGContext();
#ifdef BOREALIS_USE_GXM
                    tex = nvgCreateImageRGBA(vg, imageW, imageH,
                        (isWebp ? NVG_IMAGE_DXT5 : NVG_IMAGE_DXT1) | NVG_IMAGE_LPDDR,
                        imageData);
#else
        ...
        brls::Logger::warning("request image {} {}", urlCopy, ex.what());
        brls::sync([imagePtr, requestToken] {
            if (!requestToken->load())
                Image::clear(imagePtr, requestToken);
        });
    }
...
void Image::clear(
    brls::Image* view,
    const HTTP::Cancel& expectedRequest) {
    if (!view) return;
    ...
    if (expectedRequest && it->second->isCancel != expectedRequest)
        return;

    Ref request = it->second;
    requests.erase(it);
    request->isCancel->store(true);
    if (request->image)
        request->image->ptrUnlock();
