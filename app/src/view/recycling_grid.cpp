...
void RecyclingGrid::reloadData() {
    ...
    logicalFocusIndex = defaultCellFocus;
...
void RecyclingGrid::itemsRecyclingLoop() {
    ...
    while (true) {
        ...
        // Never recycle the logical destination while Borealis is completing
        // a controller focus transfer to it.
        if (indexNavigationEnabled &&
            minCell->getIndex() == logicalFocusIndex)
            break;
    ...
    while (true) {
        ...
        // a controller focus transfer to it.
        if (indexNavigationEnabled &&
            maxCell->getIndex() == logicalFocusIndex)
            break;
...
RecyclingGridItem* RecyclingGrid::ensureCellMaterialized(size_t index) {
    if (!dataSource || index >= dataSource->getItemCount())
        return nullptr;

    if (auto* existing = getGridItemByIndex(index))
        return existing;

    // The rendered cells form one contiguous logical range. Extend that range
    // to the requested index before returning the next focus target. For D-pad
    // navigation this normally materializes exactly one adjacent row.
    if (visibleMin == UINT_MAX || contentBox->getChildren().empty())
        return nullptr;

    while (index < visibleMin) {
        if (visibleMin == 0)
            break;
        addCellAt(static_cast<size_t>(visibleMin) - 1, false);
    }

    while (index > visibleMax &&
           static_cast<size_t>(visibleMax) + 1 < dataSource->getItemCount()) {
        addCellAt(static_cast<size_t>(visibleMax) + 1, true);
    }

    return getGridItemByIndex(index);
}
...
void RecyclingGrid::setTopBoundaryFocusRoute(
    std::function<brls::View*()> route) {
    topBoundaryFocusRoute = std::move(route);
}

void RecyclingGrid::setIndexNavigationEnabled(bool enabled) {
    indexNavigationEnabled = enabled;
    logicalFocusIndex = defaultCellFocus;
}

void RecyclingGrid::forceRequestNextPage() { this->requestNextPage = false; }

brls::View* RecyclingGrid::getNextCellFocus(
    brls::FocusDirection direction,
    brls::View* currentView) {
    if (!currentView || !dataSource || dataSource->getItemCount() == 0)
        return nullptr;
    ...
    const size_t currentIndex = *((size_t*)parentUserData);
    if (currentIndex >= itemCount)
        return nullptr;

    if (indexNavigationEnabled &&
        contentBox->getAxis() == brls::Axis::ROW) {
        // The logical index is authoritative. Recycled cell instances only
        // render that index and never infer whether navigation may leave the
        // grid.
        logicalFocusIndex = currentIndex;

        auto focusIndex =
            [this, currentView, itemCount](size_t index) -> brls::View* {
                if (index >= itemCount)
                    return currentView;

                auto* cell = ensureCellMaterialized(index);
                if (!cell) {
                    brls::Logger::error(
                        "RecyclingGrid: unable to materialize logical focus index {}",
                        index);
                    return currentView;
                }
                ...
                contentBox->setLastFocusedView(cell);

                if (auto* target = cell->getDefaultFocus())
                    return target;
                return cell;
            };

        const size_t columns =
            spanCount > 0 ? static_cast<size_t>(spanCount) : 1;
        const size_t column = logicalFocusIndex % columns;

        switch (direction) {
        case brls::FocusDirection::UP:
            // This is the only exit threshold: the current logical item is
            // already in the first finite row.
            if (logicalFocusIndex < columns) {
                if (topBoundaryFocusRoute) {
                    if (auto* target = topBoundaryFocusRoute())
                        return target;
                }

                View* next = getParentNavigationDecision(
                    this, nullptr, direction);
                if (!next && hasParent())
                    next = getParent()->getNextFocus(direction, this);
                return next;
            }

            return focusIndex(logicalFocusIndex - columns);

        case brls::FocusDirection::DOWN: {
            size_t target = logicalFocusIndex + columns;

            // Preserve the column on an incomplete final row, clamping only
            // when that column does not exist.
            if (target >= itemCount) {
                const size_t finalRowStart =
                    ((itemCount - 1) / columns) * columns;
                target = finalRowStart + column;
                if (target >= itemCount)
                    target = itemCount - 1;
            }
        ...
        case brls::FocusDirection::LEFT:
            if (column > 0)
                return focusIndex(logicalFocusIndex - 1);
            break;

        case brls::FocusDirection::RIGHT:
            if (column + 1 < columns &&
                logicalFocusIndex + 1 < itemCount)
                return focusIndex(logicalFocusIndex + 1);
            break;
    ...
    // Existing recycler navigation for every grid that has not explicitly
    // enabled finite index navigation.
    if (contentBox->getAxis() == brls::Axis::ROW &&
        direction != brls::FocusDirection::LEFT &&
        direction != brls::FocusDirection::RIGHT) {
        int rowOffset = spanCount;
        if (direction == brls::FocusDirection::UP)
            rowOffset = -spanCount;

        View* rowCurrentFocus = nullptr;
        size_t rowCurrentFocusIndex =
            *((size_t*)parentUserData) + rowOffset;

        if (rowCurrentFocusIndex >= itemCount)
            rowCurrentFocusIndex -=
                *((size_t*)parentUserData) %
                static_cast<size_t>(spanCount);

        while (!rowCurrentFocus &&
               rowCurrentFocusIndex >= 0 &&
               rowCurrentFocusIndex < itemCount) {
            for (auto* item : contentBox->getChildren()) {
                if (*((size_t*)item->getParentUserData()) ==
                    rowCurrentFocusIndex) {
                    rowCurrentFocus =
                        item->getDefaultFocus();
                    break;
                }
            }
            rowCurrentFocusIndex += rowOffset;
        }

        if (rowCurrentFocus) {
            itemsRecyclingLoop();
            return rowCurrentFocus;
        }
    }

    if (contentBox->getAxis() == brls::Axis::ROW) {
        int position =
            *((size_t*)parentUserData) % spanCount;
    ...
    if ((contentBox->getAxis() == brls::Axis::ROW &&
            direction != brls::FocusDirection::LEFT &&
            direction != brls::FocusDirection::RIGHT) ||
        (contentBox->getAxis() == brls::Axis::COLUMN &&
    ...
    while (!currentFocus &&
        ...
        for (auto* item : contentBox->getChildren()) {
            if (*((size_t*)item->getParentUserData()) ==
                currentFocusIndex) {
                currentFocus = item->getDefaultFocus();
                break;
            }
        }
    ...
    currentFocus = getParentNavigationDecision(
        this, currentFocus, direction);
    if (!currentFocus && hasParent())
        currentFocus = getParent()->getNextFocus(
            direction, this);
    return currentFocus;
}
