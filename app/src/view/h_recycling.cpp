...
bool ownsCurrentFocus(brls::View* candidate) {
...
void HRecyclerFrame::cellsRecyclingLoop() {
    ...
    while (true) {
        ...
        // A focused cell must remain alive even when analog scrolling moves it
        // outside the viewport. It will be recycled after focus moves away.
        if (ownsCurrentFocus(minCell)) break;
    ...
    while (true) {
        ...
        // Never detach the view that currently owns Borealis focus. Doing so
        // leaves the cyan cursor pointing at recycled memory and can lock or
        // crash navigation.
        if (ownsCurrentFocus(maxCell)) break;
