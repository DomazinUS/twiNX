...
Analytics::Analytics() {
    // twiNX does not enable upstream analytics. Keep this implementation inert
    // unless a downstream build explicitly provides both values.
    if (GA_ID.empty() || GA_KEY.empty()) return;
