...
public:
    ...
    // Route UP from the first logical row to a control outside the recycler.
    // The callback is evaluated at navigation time so callers can return the
    // currently active tab without attaching state to recycled cells.
    void setTopBoundaryFocusRoute(std::function<brls::View*()> route);

    // Enable finite index arithmetic for controller navigation. This is
    // opt-in so unrelated grids keep their existing navigation behaviour.
    void setIndexNavigationEnabled(bool enabled);
...
private:
    ...
    // Authoritative logical selection for row/column navigation. This is
    // independent from whichever recycled cell instances are currently alive.
    size_t logicalFocusIndex = 0;
    bool indexNavigationEnabled = false;

    std::function<brls::View*()> topBoundaryFocusRoute = nullptr;
    ...
    // Make a logical item available synchronously before Borealis transfers
    // focus. The recycler remains lazy: only the contiguous gap to the target
    // is materialized, normally one adjacent row.
    RecyclingGridItem* ensureCellMaterialized(size_t index);
